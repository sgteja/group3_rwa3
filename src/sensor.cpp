...
void AriacSensorManager::breakBeam1Callback(const osrf_gear::Proximity::ConstPtr & msg) {
    if (msg->object_detected) {  // If there is an object in proximity.
      ROS_ERROR_STREAM("Break beam 1 triggered.");
        break_beam_1_ = true;
    // } else {
        // break_beam_1_ = false;
    }
...
void AriacSensorManager::breakBeam2Callback(const osrf_gear::Proximity::ConstPtr & msg) {
    if (msg->object_detected) {  // If there is an object in proximity.
        ROS_ERROR_STREAM("Break beam 2 triggered.");
        // convBelt_Sensor_.setConveyorPower(0.0);
        // ros::Duration(5.0).sleep();
        // convBelt_Sensor_.setConveyorPower(100.0);
      break_beam_2_ = true;
    } else {
        break_beam_2_ = false;
    }
